...
// --- CONFIGURATION ---
const char* ssid = "Anuraahan's-Pixel";
const char* password = "iambored1";
const char* serverUrl = "http://10.14.109.68/update";
...
float tempOffset = -6.5;            // Heavy offset due to ENS160 heater
int cycleTime = 2000;               // 2.0 second update interval

// --- BOOTUP WARM-UP SETTINGS ---
// 180000 ms = 3 minutes. The ENS160 internal hotplate requires this time.
  ...
  if (millis() - lastCycle >= cycleTime) { 
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (httpResponseCode > 0) {
        if (!isWarmingUp) setPixel(0, 40, 0); // Clean Green
      } else {
